...
class ChatBackend:
    ...
    def write_mail(self, msg):
        if len(msg) <= 48:
            pkt = self.myFeed.write_48B(msg + bytes(48 - len(msg)), self.mySign)
            bufs = [pkt.wire]
        else:
...
class ChatFrontend:
    ...
    def cmd_sync(self, arglst):  # fetch msg from backend
        oldcount = len(self.backend.msgs)
        msgs = self.backend.read_mail()
        if len(msgs) == oldcount:
    ...
    def loop(self):
        ...
        while True:
            ...
            cmd = cmd.split(" ")
            if len(cmd) == 1 and cmd[0] == '': continue  # Empty command, reprint "tinyChat>"
            lst = [c for c in self.cmd_table if c.startswith(cmd[0])]  # find corr. command ("startwith" is enough)
            if lst == []:  # if non match, parse as int and select the corr. message
                try:
                    i = int(cmd[0])
                except:
...
if __name__ == '__main__':
    ...
    with open(path + '/config.json') as f:
        config = json.load(f)
    for i in config:
        if not i in ['alias', 'name']:  # decode keys
            config[i] = util.fromhex(config[i])
    alias = {util.fromhex(k): v for k, v in config['alias'].items()}

    faces = [io.UDP_MULTICAST(('224.1.1.1', 5000))]
    repo = repository.REPO(path, mkverifyfct(config['secret']))
    nd = node.NODE(faces, repo)
    nd.start()
    backend = ChatBackend(path, nd, config['feedID'],
                          mksignfct(config['secret']), alias)
