...
class REPO:
    ...
    def mk_child_log(self, parentFid, parentSign, childFid, childSign):
        ...
        pkt = p.write_typed_48B(packet.PKTTYPE_mkchild,
                                childFid + bytes(16),
                                parentSign)
        buf48 = pkt.fid + pkt.seq.to_bytes(4,'big') + pkt.wire[-12:]
        return self.mk_generic_log(childFid, packet.PKTTYPE_ischild,
                                   buf48, childSign, pkt.fid, pkt.seq)

    def mk_continuation_log(self, prevFid, prevSign, contFid, contSign):
        p = self.repo.get_log(prevFid)
        pkt = p.write_typed_48B(packet.PKTTYPE_contdas, pk+bytes(16), prevSign)
        buf48 = pkt.fid + pkt.seq.to_bytes(4,'big') + pkt.wire[-12:]
        return self.mk_generic_log(contFid, packet.PKTTYPE_iscontn,
                                   buf48, contSign)
...
class LOG:
    ...
    def __getitem__(self, seq):
        if seq > self.frontS: raise IndexError
        if seq < 0:
            seq = self.frontS + seq + 1
            if seq < 0: raise IndexError
