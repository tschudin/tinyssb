...
class AppTestCase(unittest.TestCase):
    ...
    def test_create_inst(self):
        ...
        self.app.add_remote(inst_id, self.key1, self.key2)
    ...
    def test_add_remote(self):
        ...
        self.app.add_remote(ret, self.key1, self.key2)
        expected = { ret: { 'il': self.app.instances.get(ret).get('il'),
            'w': { util.hex(self.key2):  { 'ir': self.key1, 'r': self.key1 } } } }
        expected[ret]['l'] = expected[ret]['il']
        self.assertEqual(expected, self.app.instances)

        self.app.add_remote(ret, self.key3, self.key2)
    ...
    def test_update(self):
        ...
        self.app._update_inst(inst_id, self.key4, self.key2)
        expected[inst_id]['w'][util.hex(self.key2)]['r'] = self.key4
        self.assertEqual(expected, self.app.instances)

        self.app._update_inst(inst_id, self.key5, self.key2, "A name", self.key3)
        expected[inst_id]['w'][util.hex(self.key2)]['r'] = self.key5
    ...
    def test_load(self):
        inst_id = self.app.create_inst()
        self.app.add_remote(inst_id, self.key1, self.key2)
        self.app._update_inst(inst_id, self.key4, self.key2, "A name", self.key3)
        self.app._update_inst(inst_id, self.key5, self.key2)
        self.app._update_inst(inst_id, None, None, "Another name", self.key6)
    ...
    def test_add_multiple_players(self):
        ...
        self.app._update_inst(inst_id, self.key3, self.key4)
        expected[inst_id]['w'][util.hex(self.key4)] = { 'ir': self.key3, 'r': self.key3 }
        self.assertEqual(expected, self.app.instances)

        self.app._update_inst(inst_id, self.key5, self.key6, "A name", self.key3)
        expected[inst_id]['w'][util.hex(self.key6)] = { 'ir': self.key5, 'r': self.key5 }
        expected[inst_id]['n'] = "A name"
        expected[inst_id]['l'] = self.key3
        self.assertEqual(expected, self.app.instances)
